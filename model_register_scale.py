...
class Attention(ViTBase, nn.Module):
    def setup(self):
        ...
        first_layer_std = (2 / 5 / self.dim) ** 0.5
        last_layer_std=first_layer_std/ (2*self.layers)**0.5

        self.wq = DenseGeneral((self.heads, self.head_dim), kernel_init=nn.initializers.truncated_normal(first_layer_std))
        self.wk = DenseGeneral((self.heads, self.head_dim), kernel_init=nn.initializers.truncated_normal(first_layer_std))
        self.wv = DenseGeneral((self.heads, self.head_dim), kernel_init=nn.initializers.truncated_normal(first_layer_std))
        self.wo = DenseGeneral(self.dim, axis=(-2, -1), kernel_init=nn.initializers.truncated_normal(last_layer_std))
...
class FeedForward(ViTBase, nn.Module):
    def setup(self):
        ...
        self.w1 = Dense(self.hidden_dim,kernel_init=nn.initializers.truncated_normal(first_layer_std))
        self.w2 = Dense(self.dim, kernel_init=nn.initializers.truncated_normal(last_layer_std))
        self.drop = nn.Dropout(self.dropout)
